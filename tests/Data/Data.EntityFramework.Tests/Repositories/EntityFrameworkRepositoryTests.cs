...
public class EntityFrameworkRepositoryTests
{
    ...
    public async Task FindAsync_ShouldReturnEntity()
    {
        ...
        // Assert
        await Assert.That(result.TryGetValue(out var found));
        await Assert.That(found.Id).IsEqualTo(1);
    }

    [Test]
    public async Task FindAsync_ShouldReturnFailure_WhenNotExists()
    {
        // Arrange
        using TestDbContext context = new(CreateOptions());
        TestRepository repository = new(context);
        ...
        // Assert
        await Assert.That(result.TryGetError(out var error));
    }
    ...
    public async Task DeleteAsync_ShouldRemoveEntity()
    {
        ...
        await Assert.That(await context.TestEntities.FindAsync(1)).IsNull();
    }
}
