...
/// <summary>
/// LARC031 - Detects when a Minimal API endpoint returns a Result or Result of T
...
{
    public const string DiagnosticId = "LARC031";
    ...
    private static void AnalyzeLambda(SyntaxNodeAnalysisContext context)
    {
        ...
        if (symbol == null || !IsMapMethod(symbol)) return;

        // 1. Try Semantic approach (Best)
        var typeInfo = context.SemanticModel.GetTypeInfo(lambda);
        ITypeSymbol? returnType = null;
        if (typeInfo.ConvertedType is INamedTypeSymbol delegateType && delegateType.DelegateInvokeMethod != null)
        {
        ...
        if (returnType != null && IsResultType(returnType))
        {
            isResultReturn = true;
        }
        else if (lambda.ExpressionBody != null)
        {
            ...
            if (bodyType != null && IsResultType(bodyType))
            {
                isResultReturn = true;
            }
        }
        ...
        if (isResultReturn)
        {
            ...
            if (returnType != null && IsHttpResultType(returnType)) return;
            
            // Check syntax for .ToEndpointResult() call as a final guard
            if (lambda.ToString().Contains(".ToEndpointResult(")) return;
    ...
    {
        string[] mapMethods = ["MapGet", "MapPost", "MapPut", "MapDelete", "MapPatch", "MapMethods"];
        if (!mapMethods.Contains(symbol.Name)) return false;
        ...
        return symbol.ReceiverType?.Name == "IEndpointRouteBuilder" || 
               ...
               (symbol.Parameters.Length > 0 && symbol.Parameters[0].Type.Name == "IEndpointRouteBuilder");
    }

    private static bool IsResultType(ITypeSymbol type)
    {
        var fullName = type.ToDisplayString();
        // Handle both Result and Result<T> by checking the start of the full name
        if (fullName.StartsWith("LightningArc.Results.Result")) return true;
        
        // Also check interfaces just in case it's a derived type or interface implementation
        if (type.AllInterfaces.Any(i => i.ToDisplayString().StartsWith("LightningArc.Results.IResult"))) return true;

        return type.BaseType != null && IsResultType(type.BaseType);
    }
    ...
    private static bool IsHttpResultType(ITypeSymbol type)
    {
        ...
        if (fullName.Contains("LightningArc.Results.AspNetCore.EndpointResult") || 
            fullName.Contains("Microsoft.AspNetCore.Http.IResult")) return true;
        
        if (type.AllInterfaces.Any(i => i.ToDisplayString().Contains("Microsoft.AspNetCore.Http.IResult"))) return true;

        return type.BaseType != null && IsHttpResultType(type.BaseType);
    }

    private static bool IsLightningArcNamespace(INamespaceSymbol? @namespace)
    {
        if (@namespace == null) return false;
        return @namespace.ToDisplayString().StartsWith("LightningArc");
    }
}
