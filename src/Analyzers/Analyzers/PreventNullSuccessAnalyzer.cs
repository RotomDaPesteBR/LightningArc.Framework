...
/// <summary>
/// LARC013 - Detects when Result.Success(null) is called for a non-nullable reference type.
/// </summary>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public class PreventNullSuccessAnalyzer : DiagnosticAnalyzer
{
    public const string DiagnosticId = "LARC013";
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        string methodName = memberAccess?.Name.Identifier.ValueText ?? (invocation.Expression as IdentifierNameSyntax)?.Identifier.ValueText ?? "";
        if (methodName != "Success" && methodName != "Ok") return;

        var symbol = context.SemanticModel.GetSymbolInfo(invocation, context.CancellationToken).Symbol as IMethodSymbol;
        if (symbol == null) return;

        // Check if the containing type is Result or Result<T>
        if (!IsResultType(symbol.ContainingType)) return;
        ...
        if (!typeArgument.IsReferenceType) return;

        // If the type is explicitly nullable, it's fine
        if (typeArgument.NullableAnnotation == NullableAnnotation.Annotated) return;
        ...
        {
            var diagnostic = Diagnostic.Create(Rule, argument.GetLocation(), typeArgument.Name);
            context.ReportDiagnostic(diagnostic);
        }
    }

    private static bool IsNullLiteralOrDefault(ExpressionSyntax expression, SemanticModel semanticModel)
    {
        // Remove casts and parentheses
        while (expression is CastExpressionSyntax cast) expression = cast.Expression;
        ...
        if (expression.IsKind(SyntaxKind.DefaultLiteralExpression)) return true;
        if (expression is DefaultExpressionSyntax) return true;
