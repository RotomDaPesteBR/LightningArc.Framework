...
public static class ExceptionMapper
{
    ...
    public static Error Map(Exception exception)
    {
        ...
        // Default fallback
        return Error.Application.Internal(LocalizationManager.GetErrorString("Application_InternalError"));
    }

    private static void RegisterDefaultMappers()
    {
        // Validation
        Register<ArgumentNullException>(ex => CreateValidationError(
            ex.ParamName, 
            "Validation_MissingField_WithParam", 
            "Validation_MissingField",
            Error.Validation.MissingField));
        
        Register<ArgumentOutOfRangeException>(ex => CreateValidationError(
            ex.ParamName, 
            "Validation_ValueOutOfRange_WithParam", 
            "Validation_ValueOutOfRange",
            Error.Validation.ValueOutOfRange));
        
        Register<ArgumentException>(ex => CreateValidationError(
            ex.ParamName, 
        ...
        Func<string?, IEnumerable<ErrorDetail>?, Error> factory)
    {
        if (!string.IsNullOrWhiteSpace(paramName))
        {
            ...
            try {
                string message = string.Format(LocalizationManager.CurrentCulture, template, paramName);
